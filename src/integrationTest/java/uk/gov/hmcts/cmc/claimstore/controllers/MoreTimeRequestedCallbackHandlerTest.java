...
public class MoreTimeRequestedCallbackHandlerTest extends BaseMockSpringTest {
    ...
    public void setup() {
        given(userService.getUserDetails(AUTHORISATION_TOKEN))
            .willReturn(SampleUserDetails.builder().withRoles(Role.CASEWORKER.getRole()).build());
    }
    ...
    public void shouldReturnWithNoValidationErrorsOnAboutToStartIfAvailable() throws Exception {
        ...
        AboutToStartOrSubmitCallbackResponse response = jsonMappingHelper.deserializeObjectFrom(
            mvcResult,
    ...
    private ResultActions makeRequest(
        ...
        CaseDetails caseDetails = CaseDetails.builder()
            ...
            .state(OPEN.getValue())
            .build();
        caseDetails.getData().put("responseDeadline", responseDeadline);
        caseDetails.getData().put("moreTimeRequested",
            moreTimeRequestedAlready ? CCDYesNoOption.YES.name() : CCDYesNoOption.NO.name()
        );

        if (respondedAlready) {
            caseDetails.getData().put("respondedAt", LocalDateTimeFactory.nowInUTC());
        }
